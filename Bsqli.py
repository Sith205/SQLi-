...
\______   \/   _____/\_____  \ |    |   |__|
 |    |  _/\_____  \  /  / \  \|    |   |  |
 |    |   \/        \/   \_/.  \    |___|  |
 |______  /_______  /\_____\ \_/_______ \__|
        \/        \/        \__>       \/   
                made by: BLACKHAT
                YOUTUBE: ProwlSec
        """ + Color.RESET)
        ...
        verbose_input = input(Color.PURPLE + "☠️ Enable verbose mode? (y/n): " + Color.RESET).strip().lower()
        ...
        print(f"\n{Color.PURPLE}⚠️ Starting scan...{Color.RESET}")
        ...
        try:
                ...
                for url in urls:
                    for payload in payloads:
                        ...
                        if success and status_code and response_time >= 10:
                            ...
                            if self.verbose:
                                print(f"{Color.GREEN}✅ SQL Injection Detected! URL: {url_with_payload} - Response Time: {response_time:.2f}s - Status: {status_code}{Color.RESET}")
                            else:
                                print(f"{Color.GREEN}✅ Vulnerable URL Found: {url_with_payload}{Color.RESET}")
                        else:
                            if self.verbose:
                                print(f"{Color.RED}❌ Not Vulnerable: {url_with_payload} - Response Time: {response_time:.2f}s - Status: {status_code}{Color.RESET}")
            ...
            print(f"{Color.YELLOW}⚠️ Scan interrupted by user.{Color.RESET}")

        print(f"\n{Color.CYAN}📊 Scan Complete.{Color.RESET}")
        print(f"{Color.YELLOW}Total Tests Performed: {self.total_tests}{Color.RESET}")
        print(f"{Color.GREEN}SQL Injection Found: {self.vulnerabilities_found}{Color.RESET}")
        if self.vulnerabilities_found > 0:
            print(f"{Color.GREEN}✓ {self.vulnerabilities_found} Vulnerability(s) Found!{Color.RESET}")
        else:
            print(f"{Color.RED}✗ No vulnerabilities found. Keep hunting!{Color.RESET}")
        ...
        print(f"{Color.CYAN}🔥 Thank you for using the BSQLi tool by BLACKHAT! Don't forget to check out our YouTube: ProwlSec{Color.RESET}")
